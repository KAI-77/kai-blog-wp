# Auto Blog Demo

**Auto Blog Demo** is a WordPress plugin I built to automatically generate draft blog posts with topic-based content and images. It's designed to save time and make content creation easy, especially if you want to populate your WordPress site with realistic-looking posts for testing or demonstration purposes.

---

## Features

- Select from multiple topics: **Coloring, DIY, Art, Recipes, Fitness, Travel, Tech**.
- Generate **single or multiple posts** with a click.
- Randomly picks a **title** and corresponding **paragraphs** that flow together naturally.
- Attaches **main images and extra images** for visual appeal.
- Sets posts to **Blog category**, assigns **tags**, and adds a **summary** automatically.
- Draft posts are ready to **preview** before publishing.
- Lightweight, secure, and easy to use with **nonce verification**.

---

## Installation

1. Download or clone this repo.
2. Copy the plugin folder into your WordPress `wp-content/plugins/` directory.
3. Activate the plugin from the WordPress **Plugins** page.

---

## Usage

1. Go to **Auto Blog Demo** in the WordPress admin menu.
2. Select a **topic** from the dropdown.
3. Click **Generate 1 Post** to create a single draft, or **Generate 3 Posts** to bulk-generate multiple drafts.
4. After generation, a **success message** appears with a link to **preview** each post.
5. All posts are saved as **drafts** so you can edit before publishing.

---

## Notes

- Paragraphs are randomly shuffled for variety, so repeated generations produce unique content.
- Designed for **demonstration or testing purposes**; tweak the content arrays to suit your actual site needs.

---

## Contributing

I built this plugin for simplicity and demonstration, but feel free to fork, tweak, or extend it. Pull requests or suggestions are welcome!

---

## License

MIT License – free to use and modify.
